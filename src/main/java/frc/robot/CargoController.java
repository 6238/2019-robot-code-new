...
public class CargoController implements RobotController {
    ...
    //the point of this is to get the relative distance between when the arm is retracted and extended 
    public CargoController() {
        int count;
        double distance;
        cargoMech = new Encoder(0, 1, false, Encoder.EncodingType.k4X);
        
        //these lines initialize different parameters of the encoder, but aren't used to control anything so far
        ...
        //these lines display the encoder distance on the shuffleboard for testing purposes, it isn't necessary for competition
        count = cargoMech.get();
    ...
    //this constant represents the relative distance between when the arm is up and down
    private final double turnMax = 10; //it needs to be updated experimentally
    ...
    public void moveCargoMech(RobotProperties properties) {
        ...
        //this sets isSet to false which is used in the following statement to control the arm
        ...
        if (!isSet) {
            ...
            if (isReleased) {
                //if it is in the down position, we keep turning it until the distance returns to 0
                if(cargoMech.getDistance() != 0)
                {
                    cargoPivot.set(-5);
                }

                //once the distance is 0, isReleased is set to false, representing the up position, 
                //and isSet is set to true representing that the arm is currently either in an up or down position
                else
                {
                    isSet = true;
                    cargoPivot.set(0);
                ...
                //these statements represent the same mechanism as the previous if statements, except it turns the
